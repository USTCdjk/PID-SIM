...
from .common import conv, fft2d, fftshift2d, Upsampler
...
class PCALayer(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        x_fft = fft2d(x)
        x_phase = torch.atan2(x_fft.imag + 1e-8, x_fft.real + 1e-8) #算一组点计算点(x，y)组成的向量的弧度，该弧度是与x轴正方向的弧度
